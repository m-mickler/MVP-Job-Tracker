...
$userForm.submit((event) => {
    ...
    fetch( 
        ...
        .then((data) => {
            console.log(data);
            $heading.empty();
            let newText = $(`<strong>Remember this unique ID - ${data[0].id}</strong>`)
...
//SELECTS A USER AND DISPLAYS THEIR UNIQUE ID
...
// DELETES A JOB LISTING BY ID AND COMPANY NAME
...
$search.submit((event) => { 
    ...
    $.ajax({
        type: "GET",
        url: `/api/users/${firstname}/${lastname}`,
        success: res => {
            alert(`Here is your ID - ${res.id}`)
            $heading.empty();
            let newText = $(`<strong>Here is your ID - ${res.id}</strong>`)
            newText.appendTo($heading);
            console.log(res)
        },
        contentType: "application/json"
